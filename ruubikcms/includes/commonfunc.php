...
function clean_url($pageurl, $table = 'page')
{
    ...
    if ($level > 1) {
        $mother = query_prep('SELECT mother FROM ' . $table . ' WHERE pageurl = ?', [$pageurl]);
    }
    ...
    if ($level <= 1) {
        ...
        $urlrest = query_prep('SELECT mother FROM ' . $table . ' WHERE pageurl = ?', [$mother]) . '/' . $mother . '/' . $pageurl . $url_suffix;
    } else {
        return false;
    }
...
{
    echo query_prep('SELECT content FROM snippet WHERE name = ?', [$name]);
}//end snippet()

// --- Prints html content and evaluates php code for a given snippet.
function snippet_php($name)
{
    eval('?>' . query_prep('SELECT content FROM snippet WHERE name = ?', [$name]));
}//end snippet_php()
