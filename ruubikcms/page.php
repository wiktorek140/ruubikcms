...
if (basename((string) $_SERVER['REQUEST_URI']) == 'page.php') {
    die('Access denied');
}
...
if (!$page['image1'] || $page['image1'] == "") {
    ...
    if ($site['no_image1'] == 1) {
        $page['image1'] = frontpage_value('image1');
    }
...
if (!$page['image2'] || $page['image2'] == "") {
    ...
    if ($site['no_image2'] == 1) {
        $page['image2'] = frontpage_value('image2');
    }
    // otherwise use transparent 1px dummy gif
    else {
...
if ($page['levelnum'] == 2) {
    ...
    $p = query_single("SELECT mother FROM page WHERE pageurl = '" . $page['mother'] . "'");
    // a.k.a grandmother
...
// has page sub pages?
if ($page['levelnum'] < 3) {
    if (query_single("SELECT name FROM page WHERE mother = '" . $page['pageurl'] . "'") == null) {
        $page['has_sub_pages'] = false;
    } else {
        $page['has_sub_pages'] = true;
    }
}
...
foreach ($dbh->query($sql) as $row) {
    ...
    if ($clean_url) {
        $url = $siteroot . 'index.php/' . $row['pageurl'] . $url_suffix;
    } else {
...
if ($stmt->execute([$p])) {
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        if ($clean_url) {
            $url = $siteroot . 'index.php/' . $p . '/' . $row['pageurl'] . $url_suffix;
        } else {
            $url = 'index.php?p=' . $row['pageurl'];
        }
...
if ($stmt->execute([$p])) {
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        if ($clean_url) {
            $url = $siteroot . 'index.php/' . $page['mother'] . '/' . $_GET['p'] . '/' . $row['pageurl'] . $url_suffix;
        } else {
            $url = 'index.php?p=' . $row['pageurl'];
        }
...
*/
$page['dropdownmenu'] = '<ul id="nav">';
// Edit id for navigation div here (id="dropdownMenu" etc.)
$sql = "SELECT pageurl, name FROM page WHERE levelnum = 1 AND status = 1 ORDER BY ordernum";
foreach ($dbh->query($sql) as $row) {
    $page['dropdownmenu'] .= '<li' . ($_GET['p'] == $row['pageurl'] ? ' class="selected"' : '') . '><span><a href="' . ($clean_url ? clean_url($row['pageurl']) : 'index.php?p=' . $row['pageurl']) . '">' . $row['name'] . '</a></span>';
