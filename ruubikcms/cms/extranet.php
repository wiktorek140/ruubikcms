...
if (isset($_POST['save'])) {
    ...
    if ($_POST['creator'] == "") {
        $_POST['creator'] = $_SESSION['uid'];
    }

    // check rights to save
    if ($_SESSION['level'] < 4 and $_POST['creator'] != $_SESSION['uid']) {
        $error = NOTALLOWED;
    }
    ...
    if ($_POST['mother'] === null) {
        $_POST['mother'] = $_POST['mother_hidden'];
    }

    // get level number
    if ($_POST['mother'] == '---notinmenu---') {
        $levelnum = 0;
    } else if ($_POST['mother'] and $_POST['mother'] != '') {
        $levelnum = query_prep("SELECT levelnum + 1 FROM extrapage WHERE pageurl = ?", [$_POST['mother']]);
    } else {
        $levelnum = 1;
    }
    ...
    if (!isset($_GET['p']) and isset($_GET['n'])) {
        // echo $_POST['pageurl'];
        if ($_POST['pageurl']) {
            $newpageurl = get_unique_url($_POST['pageurl'], 4); // make pageurl from URL if given
        } else {
            $newpageurl = get_unique_url($_POST['name'], 4); // end if
        }
    ...
    // page already exists
    if (isset($_GET['p'])) {
        $oldlevel = query_prep("SELECT levelnum FROM extrapage WHERE pageurl = ?", [$_GET['p']]);
        $oldmother = query_prep("SELECT mother FROM extrapage WHERE pageurl = ?", [$_GET['p']]);
    ...
    if (isset($_GET['n']) and !isset($_POST['header1'])) {
        $_POST['header1'] = $_POST['name'];
    }
