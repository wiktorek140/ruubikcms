...
if (isset($_POST['save'])) {
    ...
    if ($_POST['mother'] == '---notinmenu---') {
        ...
        $levelnum = query_prep("SELECT levelnum + 1 FROM page WHERE pageurl = ?", [$_POST['mother']]);
    } else {
        $levelnum = 1;
    }
    ...
    if (!isset($_GET['p']) and isset($_GET['n'])) {
        ...
        if (!empty($_POST['pageurl'])) {
            $newpageurl = get_unique_url($_POST['pageurl'], 0); // make pageurl from URL if given
        } else {
            $newpageurl = get_unique_url($_POST['name'], 0); // end if
        }
    ...
    // page already exists
    if (isset($_GET['p'])) {
        $oldlevel = query_prep("SELECT levelnum FROM page WHERE pageurl = ?", [$_GET['p']]);
        $oldmother = query_prep("SELECT mother FROM page WHERE pageurl = ?", [$_GET['p']]);
