...
if (isset($_POST['import'])) {
        ...
        foreach (explode("\n", $csvcontent) as $line) {
            ...
            if ($lines > 0) {
                if (count($linearray) == 9) {
                    ...
                    $stmt = $dbh->prepare(
                        'INSERT OR REPLACE INTO extrauser (username, password, firstname, lastname, organization, email, phone, active, expirytime) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
                    );
    ...
    $csvdata = [];
    $csvdata = [0 => "username;password;firstname;lastname;organization;email;phone;active;expirydate (yyyy-mm-dd)\n"];
    $line = 1;
    $result = $dbh->query(
        'SELECT username, firstname, lastname, organization, email, phone, active, expirytime FROM extrauser ORDER BY username',
    );
    ...
    exit();
} else if (isset($_POST['save']) and !isset($_POST['search'])) {
    if (!valid_csrf_token($_POST['token'])) {
        die(NOTALLOWED);
    }
    ...
    if (!$_POST['username']) {
        $_POST['username'] = $_POST['username_hidden'];
    }
    ...
    if (!isset($error)) {
        ...
        if ($_POST['password'] == '') {
            ...
            $stmt = $dbh->prepare(
                'UPDATE extrauser SET firstname = ?, lastname = ?, email = ?, phone = ?, active = ?, expirytime = ?, organization = ? WHERE username = ?',
            );
...
if (query_single('SELECT COUNT(*) FROM extrauser') != 0) {
    if (isset($_GET['p'])) {
        ...
        $stmt = $dbh->prepare(
            'SELECT username, firstname, lastname, email, phone, active, expirytime, organization FROM extrauser WHERE username = ?',
        );
                    ...
                    <div id="rightContent">
                        <div id="newsAdmin">
                            ...
                            if (isset($_POST['search']) and !empty($_POST['keyword'])) {
                                $stmt = $dbh->prepare(
                                    "SELECT username, firstname, lastname, active, expirytime, organization FROM extrauser WHERE username LIKE '%' || ? || '%' OR firstname LIKE '%' || ? || '%' OR lastname LIKE '%' || ? || '%' OR organization LIKE '%' || ? || '%' ORDER BY username",
                                );
...
?> /></td>
                                    ...
                                    <input name="username_hidden" type="hidden" value="<?php
                                    if (isset($_GET['p'])) {
                                        echo ec($_GET['p']);
                                    }
                                    ?>" />
                                ...
                                <tr>
                                    ...
                                    <td class="tdNewsAdminCenter">
                                        <select name="active">
                                            <option value="1" <?php
                                            if ($user['active'] == 1 || !isset($_GET['p'])) {
                                                echo 'selected="selected"';
                                            }
                                            ?>><?php echo ENABLED; ?></option>
                                            <option value="0" <?php
                                            if ($user['active'] == 0 && isset($_GET['p'])) {
                                                echo 'selected="selected"';
                                            }
                                            ?>><?php echo DISABLED; ?></option>
                                        </select>
                                    </td>
                            ...
                            <?php } else {// pagination
                                ...
                                if ($ordercode == 1) {
                                    ...
                                    $order = 'username'; // end if
                                }

                                if (isset($_GET['desc']) and $_GET['desc'] == 1) {
                                    $desc = ' DESC';
                                } else {
                                    $desc = '';
                                }
