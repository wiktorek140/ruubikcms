...
foreach ($dbh->query($sql) as $row) {
    ...
    $row['username'] .
    '"' .
    ($row['username'] == $_GET['p'] ? ' class="selected"' : '') .
    '>' .
    $row['username'] .
    '</a></div></div>';
}
echo '</div><div class="newsButton"><a href="users.php?role=4">' .
  SUPEREDITORS .
  '</a></div><div class="subMenu1"' .
  ($_GET['role'] == 4 ? ' id="open"' : '') .
  '>';
...
echo '<div class="newsButton"><a href="users.php?role=3">' .
  PUBLISHERS .
  '</a></div><div class="subMenu1"' .
  ($_GET['role'] == 3 ? ' id="open"' : '') .
  '>';
...
echo '</div>';
echo '<div class="newsButton"><a href="users.php?role=2">' .
  CONTRIBUTORS .
  '</a></div><div class="subMenu1"' .
  ($_GET['role'] == 2 ? ' id="open"' : '') .
  '>';
