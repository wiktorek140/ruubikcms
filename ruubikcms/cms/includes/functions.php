...
{
    $numrows = query_prep("SELECT COUNT(*) FROM page WHERE pageurl = ?", [$pageurl]);
    if ($numrows != 0) {
        return true;
    } else {
        return false;
...
{
    $numrows = query_prep("SELECT COUNT(*) FROM extrapage WHERE pageurl = ?", [$pageurl]);
    if ($numrows != 0) {
        return true;
    } else {
        return false;
...
{
    $numrows = query_prep("SELECT COUNT(*) FROM snippet WHERE name = ?", [$name]);
    if ($numrows != 0) {
        return true;
    } else {
        return false;
...
{
    $numrows = query_prep("SELECT COUNT(*) FROM cmsuser WHERE username = ?", [$name]);
    if ($numrows != 0) {
        return true;
    } else {
        return false;
...
{
    $numrows = query_prep("SELECT COUNT(*) FROM extrauser WHERE username = ?", [$name]);
    if ($numrows != 0) {
        return true;
    } else {
        return false;
...
function get_unique_url($str, $type)
{
    ...
    for ($j = 0; $j < strlen($str); $j++) {
        ...
        if (ord($str[$j]) >= 48 && ord($str[$j]) <= 57) {
            $newStr .= $str[$j];
            // add to string if ASCII value equals to numbers 0-9
        }

        if (ord($str[$j]) >= 97 && ord($str[$j]) <= 122) {
            $newStr .= $str[$j];
            // add to string if ASCII value equals to "normal" lowercase letters
        }

        if ($type == 2 || $type == 3) {
            if (ord($str[$j]) == 64 || ord($str[$j]) == 46) {
                $newStr .= $str[$j];
                // in usernames, allow "." OR "@" for email addresses
            }
        }
    }//end for
    ...
    if ($str == '---notinmenu---') {
        $str = 'notinmenu';
        // '---notinmenu---' is used to select free page
    }

    if ($str == 'index') {
        $str = 'index1';
        // 'index' does not work with cleanurl & front page uri just index.php
    }
