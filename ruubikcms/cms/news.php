...
if (isset($_POST['save'])) {
    ...
    if ($_GET['id']) {
        ...
        $stmt = $dbh->prepare(
            'UPDATE news SET title = ?, time = ?, text = ?, shorttext = ?, linktopage = ?, status = ?, updated = ?, updater = ? WHERE id = ?',
        );
        ...
        // insert new news
        $stmt = $dbh->prepare(
            'INSERT INTO news (title, time, text, shorttext, linktopage, status, creator) VALUES (?, ?, ?, ?, ?, ?, ?)',
        );
        $stmt->bindParam(1, $_POST['title']);
        $stmt->bindParam(2, $_POST['time']);
        $stmt->bindParam(3, $newsdata);
...
<div id="rightDiv">
      ...
      <table cellspacing="0" cellpadding="0" border="0" class="newsTable">
        <tr>
          ...
          <td class="tdNewsAdminCenter">
            ...
            ?>" /></td>
        <td class="tdNewsAdminRight">
          <a href="#" class="tooltip" title="<?php echo H_NEWSTITLE; ?>">
            <img src="images/help.gif" class="imgover" alt="" />
        </a>
      </td>
  ...
  <tr>
    ...
    <td class="tdNewsDate"><input type="text" name="time" class="date-pick" value="<?php
    if (!isset($_GET['id'])) {
        echo date('Y-m-d');
    } else {
  ...
  <td class="tdNewsAdminRight">
    <a href="#" class="tooltip" title="<?php echo H_NEWSDATE; ?>">
      <img src="images/help.gif" class="imgover" alt="" />
      </a>
</td>
</tr>
...
<td class="tdNewsAdminCenter">
<textarea name="shorttext" rows="4" cols="20"><?php
if (isset($news['shorttext'])) {
    echo ec($news['shorttext']);
}
?></textarea>
</td>
